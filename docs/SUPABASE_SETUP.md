...
4. `supabase/migrations/0004_lab_and_events.sql` — имена крыс, таймаут
   испытаний (`game_settings`), таймеры игрока и таблица `events`
   (глобальные дрейфующие события). Не разрушающая.

Разброс нескольких биомов по участку (7–7777 ячеек, покрытие ≥50%) новой
миграции не требует — размер биома это просто длина `block_ids`. Нужно лишь
пере-задеплоить функцию `game-api`.
