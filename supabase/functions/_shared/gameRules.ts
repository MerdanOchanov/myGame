...
export const MEDICINE_CONSUME_CHANCE = 0.3;
export const MIN_RATS = 3;
export const MAX_RAT_NAME_LENGTH = 24;
export const MAX_MEDICINE_NAME_LENGTH = 40;

// -------------------------------------------------------------- global events
// Admin-created, drift deterministically from seed+time (mirror of
// src/core/events/GameEvent.ts — keep in sync). Hexagonal, radius 1..10000 km.
export const MIN_EVENT_RADIUS_KM = 1;
export const MAX_EVENT_RADIUS_KM = 10000;
export const MIN_EVENT_SEVERITY = 1;
export const MAX_EVENT_SEVERITY = 3;
const DRIFT_BUCKET_MS = 60 * 60 * 1000;
const MAX_DRIFT_DEG = 35;
...
function waypointOffset(seed: string, bucket: number): { dLat: number; dLng: number } {
  ...
  return { dLat: (rand() * 2 - 1) * MAX_DRIFT_DEG, dLng: (rand() * 2 - 1) * MAX_DRIFT_DEG };
}

export function eventCenterAt(basePosition: { lat: number; lng: number }, seed: string, now: Date): { lat: number; lng: number } {
  const t = now.getTime();
  const bucket = Math.floor(t / DRIFT_BUCKET_MS);
  const frac = (t % DRIFT_BUCKET_MS) / DRIFT_BUCKET_MS;
  const a = waypointOffset(seed, bucket);
  const b = waypointOffset(seed, bucket + 1);
  let lat = basePosition.lat + a.dLat + (b.dLat - a.dLat) * frac;
  let lng = basePosition.lng + a.dLng + (b.dLng - a.dLng) * frac;
...
export function eventHexagon(center: { lat: number; lng: number }, radiusKm: number): [number, number][] {
  const earthKm = 6371;
  const angularDist = radiusKm / earthKm;
...
export function eventEffects(severity: number): ActiveEffect[] {
  ...
  return [
    ...
    { key: 'anti_infection', power: -1 * s, durationSec: 60, source: 'event' },
  ];
}
